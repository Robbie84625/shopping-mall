...
public class ProductServiceImpl implements ProductService {
  ...
  public PagedResponse<Product> getProducts(ProductQueryParams productQueryParams) {
    ...
    Sort.Direction sortDirection =
        "asc".equalsIgnoreCase(productQueryParams.getSort())
            ? Sort.Direction.ASC
            : Sort.Direction.DESC;
    String orderBy =
        productQueryParams.getOrderBy() != null ? productQueryParams.getOrderBy() : "createdDate";
    // 建立分頁和排序的設定
    Pageable pageable =
        PageRequest.of(
            productQueryParams.getPage(),
            productQueryParams.getLimit() + 1,
  ...
  private Page<Product> fetchProducts(ProductQueryParams productQueryParams, Pageable pageable) {
    ...
    switch (queryType) {
      case CATEGORY_AND_KEYWORD:
        return productRepository.findByCategoryAndProductNameContaining(
            productQueryParams.getProductCategory(), productQueryParams.getKeyword(), pageable);
      case CATEGORY_ONLY:
        return productRepository.findByCategory(productQueryParams.getProductCategory(), pageable);
      case KEYWORD_ONLY:
        return productRepository.findByProductNameContaining(
            productQueryParams.getKeyword(), pageable);
      default:
  ...
  private long countProducts(ProductQueryParams productQueryParams) {
    ...
    switch (queryType) {
      case CATEGORY_AND_KEYWORD:
        return productRepository.countByCategoryAndProductNameContaining(
            productQueryParams.getProductCategory(), productQueryParams.getKeyword());
      case CATEGORY_ONLY:
